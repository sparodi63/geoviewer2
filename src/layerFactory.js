...
var layerFactory = {
  ...
  WMS(layerConfig) {
    util.log('layerFactory - Creazione Layer WMS: ' + layerConfig.name);

    var format  = (layerConfig.geomType === 'VECTOR') ? 'image/png8' : (layerConfig.cacheMinZoomLevel) ? 'image/jpeg' : 'image/png',
        minZoom = (layerConfig.minScale) ? util.getZoomFromScaleDenom(layerConfig.minScale) : 8,
        maxZoom = (layerConfig.maxScale) ? util.getZoomFromScaleDenom(layerConfig.maxScale) : 20,

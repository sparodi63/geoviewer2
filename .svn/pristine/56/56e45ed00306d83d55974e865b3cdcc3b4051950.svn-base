...
import {layerGroup,Control} from 'leaflet'
...
GV.Buttons.search = function (btnOptions, map) {
  ...
  if (layers.length === 0) {
    throw new Error('bottone search: layer non trovati')
  }
  btnOptions.layer = layerGroup(layers)

  return new Control.Search(btnOptions)
}

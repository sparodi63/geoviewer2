...
GV.Map = L.Map.extend({
    ...
    initialize: function () {
        ...
        _.extend(this.mapOptions, GV.config.application.mapOptions);
    ...
    setLayersInRange: function () {
        ...
        this.on('zoom', function () {
            ...
            _.each(layers, function (layer) {
                GV.config.setLayerAttribute(layer.name, 'inRange', this.layerInRange(layer));
            }, this);
        });
    },
    ...
    loadBaseLayers: function () {
        ...
        _.each(GV.config.baseLayers, function (layerConfig) {
    ...
    loading: function (state, layer) {
        if (!!state) {
            if (this._spinning === 0) {
                GV.util.log('start load: ' + layer.config.name);
            ...
            this._spinning--;
            if (this._spinning <= 0) {
                GV.util.log('end load: ' + layer.config.name);

...
// var geoserverUrl = 'http://geoservizi.regione.liguria.it:8080/'

// var env = GV.utils.getUrlParam("ENV") || "TEST";
...
if (action === 'INSERT') {
  ...
  layer = 'PRATICA'
  var tipoPratica = getTipoPratica()
  if (getTipoPratica() === 'DER') layer = 'PRATICA_DE'
  if (idPunto) layer = 'PUNTO_DE'
}
  ...
  if (action === 'INSERT') {
    ...
    tools.push({
      ...
      options: {
        ...
        submit: function (data, deleted, loading) {
          var data = {
            prov: prov,
    ...
    if (mappale) {
      ...
      items.push({
        ...
        findOptions: {
          ...
          maxZoom: 18,
          cqlFilter: "CT24_COD_COM='" + codBelfiore + "' AND CT24_SEZ='" + sezione + "' AND CT24_FOGLIO='" + foglio + "' AND CT24_NUMERO='" + mappale + "'",
        },
      })
    } else {
      items.push({
